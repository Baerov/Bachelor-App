...
var scan = {
    ...
    init: function () {
        ...
        $('body').on('click', 'button#delete-selected', $.proxy(function () {
            if (window.confirm(lajax.t('Sunteți siguri că doriți să ștergeți aceste obiecte?'))) {
                this.deleteSelected();
            }
        }, this));
        $('body').on('click', 'a.delete-item', $.proxy(function (event) {
            if (window.confirm(lajax.t('Sunteți siguri că doriți să ștergeți acest obiect?'))) {
